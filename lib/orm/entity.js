...
class Entity extends Table {
  constructor(engine, name, alias = null) {
    ...
    this.attributes.set(new Attribute(engine, this, 'x'));
  ...
  prepare(entity = {}) {
    return co(function*(self, prepare){
      ...
      for (let attribute of self.attributes.getOrderedElements()) {
        yield attribute.prepare(entity, [attribute.id]);
      }
    })(this, super.prepare);
  }
  ...
  save(entity) {
    return co(function*(self, save){
      if (!entity.id) {
        // In order to get a revisionId, we must first have an entityId.
        entity.revisionId = null;
        yield save.bind(self)(entity);
      }

      // Get a new revision.
      let revision = {entityId: entity.id, date: Date.now()};
