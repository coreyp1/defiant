...
class Attribute extends Table {
  constructor(engine, relationship, name, alias=null) {
    super(engine, relationship.name + '-' + name, alias);
    this.id = name;
  ...
  createTable() {
    return co(function*(self, createTable){
      ...
      for (let attribute of self.attributes.getOrderedElements()) {
  ...
  prepare(entity, path) {
    return co(function*(self, prepare){
      ...
      for (let i in values) {
        ...
        let relationship = self.drilldown(entity, path.slice(0, -2));
        ...
        for (let attribute of self.attributes.getOrderedElements()) {
          yield attribute.prepare(entity, path.concat([attribute.id]));
        }
      }
    })(this, super.prepare);
  }
    ...
    return co(function*(self, save){
      ...
      for (let i in newValue) {
        ...
        // Update the entry (if necessary).
          ...
          if (yield self.valuesHaveChanged(oldValue[i], newValue[i])) {
      ...
      // Finally, "remove" any old values that are no longer present.
  ...
  load(entity, path) {
    return co(function*(self){
      ...
      rows.forEach(item => value[item.id] = item);

      // Load any sub-attributes.
      for (let i in value) {
        for (let attribute of self.attributes.getOrderedElements()) {
          yield attribute.load(entity, path.concat([i, attribute.id]));
        }
      }
    })(this);
  }
  ...
  purge(parentIds) {
    return co(function*(self){
      let table = sql.define(self.schema());
      let attributes = self.attributes.getOrderedElements();
      let toBeDeleted = [];
      if (attributes) {
        ...
        let db = self.engine.database;console.log(query);
        let dball = promisify(db.all, {context: db});
        let rows = yield dball(query.text, ...query.values);
        rows.forEach(item => toBeDeleted.push(item));
      }
      ...
      let stmt = self.engine.database.prepare(query.text);console.log(query);
      let run = promisify(stmt.run, {context: stmt});
      let finalize = promisify(stmt.finalize, {context:stmt});
      yield run(query.values);
      yield finalize();
