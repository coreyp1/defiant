...
class Handler {
  ...
  constructor(data={}) {
    /**
     * @member {String} Defiant.Plugin.Router.Handler#id The unique id of the
     *   Handler.
     */
    ['id',
    /**
     * @member {number} Defiant.Plugin.Router.Handler#weight The weight of the
     *   Handler.
     */
      'weight',
    /**
     * @member {String} Defiant.Plugin.Router.Handler#path The path that the
     *   Handler should match against.
     */
      'path', 'alwaysProcess', 'allowShowLink', 'allowAccess', 'showLink', 'menu', 'renderable'].map(val => this[val] = data[val] ? data[val] : (this[val] ? this[val] : this.constructor[val]));
    /**
...
/**
 * @prop {boolean} Defiant.Plugin.Router.Handler.alwaysProcess Give the request context to the handler even if
 *   another handler has already set the response body or code.
 */
Handler.alwaysProcess = false;

/**
 * @prop {boolean} Defiant.Plugin.Router.Handler.allowShowLink Permit the link to be shown even if there is
 *   not an explicit
...
/**
 * @prop {Defiant.Plugin.Theme.Renderable} Defiant.Plugin.Router.Handler.renderable The Renderable that is
 *   to be wrapped by the Theme.
 */
