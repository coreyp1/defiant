...
class Handler {
  ...
  async init(context) {
    if (this.renderable) {
      ...
      let renderable = (typeof this.renderable === 'function') ? new this.renderable() : this.renderable;
      let data = await renderable.init(context);
      context.httpResponse = new Themed(context, {
        language: 'us',
        siteName: 'Defiant',
        head: '',
        jsFooter: '',
        content: renderable.commit(context, data),
      });
    }
  }
}
