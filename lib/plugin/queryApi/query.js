...
class Query {
  ...
  async init(context, data) {
    ...
    let query = this.getQuery(context).toQuery();
    ...
    return await new Promise((accept, reject) => {
      db.all(query.text, ...query.values, (err, rows) => {
        this.rows = rows || [];
        return accept(rows ? undefined : err);
      });
    });
  }
  ...
  formatOutput(formattedRows) {
    const QueryApi = this.engine.pluginRegistry.get('QueryApi');
    const BaseFormatter = QueryApi.baseFormatRegistry.get(this.data.format.type || 'inline');
    let output = BaseFormatter.commit(this, formattedRows);
    // TODO: Escape.
