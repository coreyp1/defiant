...
class Query {
  ...
  async init(context, data) {
    ...
    for (let querySet of this.getQueryList(this.data, {})) {
      ...
      // Execute the query.
      query = query.toQuery();
      querySet.current.rows = await new Promise((accept, reject) => {
  ...
  commit(rows) {
    ...
    return this.formatOutput(this.data, rows);
  }

  formatOutput(subquery, rows) {
    let QueryApi = this.engine.pluginRegistry.get('QueryApi');
    let outRows = [];
    ...
    for (let row of rows) {
      ...
      for (let field of Object.values(subquery.statement.fields)) {
        if (field.type == 'subquery') {
          outRow[field.nameAlias] = this.formatOutput(field, row[field.nameAlias]);
        }
        else {
    ...
    const BaseFormatter = QueryApi.baseFormatRegistry.get(subquery.format.type || 'inline');
    let output = BaseFormatter.commit(subquery, outRows);
    // TODO: Escape.
    return subquery.id
