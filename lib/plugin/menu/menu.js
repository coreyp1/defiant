...
class Menu extends Plugin {
  ...
  async init() {
    ...
    let Layout = this.engine.pluginRegistry.get('Layout');
    ...
    for (let menu of menus) {
      ...
      this.menus[menu] = new MenuWidget(settings, this.engine);
      Layout.widgetRegistry.set(this.menus[menu]);
    }

    // Load the Menu Entry overrides.
    this.entryOverrides = await Settings.cacheRegistry.get('menu/entryOverrides.json').load();
