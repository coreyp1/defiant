...
class LayoutEditHandler extends AdminHandler {
  canAccess(context) {
    return co(function*(self, superCanAccess){
      if (yield superCanAccess.call(self, context)) {
        // Deny Access if the layout does not exist.
        let layoutId = context.request.urlTokenized[3]
        let layout = context.engine.plugin.get('Layout').layouts.get(layoutId);
        let layoutData = context.engine.plugin.get('Settings').cache.get(`layout/${layoutId}.json`);
