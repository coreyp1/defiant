...
class SettingsClassMapForm extends Form {
  init(context) {
    ...
    return co(function*(self, superInit) {
      ...
      let paths = yield Settings.cacheRegistry.get('settings/locations.json').load();
      let options = [];
      ...
      let classes = yield Settings.cacheRegistry.get('settings/classMapOverride.json').load();
