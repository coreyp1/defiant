...
class FormValidate extends Hidden {
  init(context) {
    ...
    this.encrypted = Cipher.encryptSigned(JSON.stringify(this.data.encrypt), JSON.stringify(this.data.static), context.request.session.fapikeyraw,  this.form.getIV(this.name + this.ivCounter));
