...
class FapiHandler extends Handler {
  init(context) {
    ...
    return co(function*(self) {
      ...
      let fileUploadEntity = context.engine.plugin.get('Orm').entity.get('fileUploadEntity');
      ...
      let decrypted = cipher.decryptSigned({tag: tag, aad: JSON.stringify(validate[0]), content: validate[1]}, context.session.fapikeyraw, dummyForm.getIV('validate' + iv));
      if (decrypted === false) {
        context.fapiError = true;
        // TODO: Translate.
        context.volatile.message.set('validateToken', 'Form validation token is broken.', 'danger');
      ...
      let buildStateJson = cipher.decryptSigned({tag: decrypted.e.buildState.t, aad: '', content: post.buildState}, context.session.fapikeyraw, dummyForm.getIV('buildState' + decrypted.e.buildState.c)),
