...
const Collection = require('../themeBase/renderable/collection');
...
class Form extends Collection {
  constructor(buildState={}, ivBase=uuid.v4()) {
    super();
    this.form = this;
  ...
  init(context) {
    ...
    return co(function*(self, superInit) {
      ...
      let validate = new (Fapi.getElement('FormValidate'))('validate', {
        encrypt,
  ...
  setError(context, elementName, message) {
    context.engine.plugin.get('Fapi').setError(context, this.name, elementName, message);
  }
