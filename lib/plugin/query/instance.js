...
class Instance {
  ...
  commit(rows) {
    let Query = this.engine.plugin.get('Query');
    let outRows = [];
    ...
    for (let row of rows) {
      ...
      for (let field of Object.values(this.data.fields)) {
        ...
        let base = Query.baseRegistry.get(baseObject.base)
  ...
  getQuery(context) {
    ...
    let Orm = this.engine.plugin.get('Orm');
    let Query = this.engine.plugin.get('Query');
    let baseObject = Object.values(this.data.bases).filter(val => !val.from)[0];
    let queryBase = Query.baseRegistry.get(baseObject.base);
    let baseSchema = Orm.entity.get(queryBase.data.orm).schema();
    ...
    for (let field of Object.values(this.data.fields)) {
      ...
      let base = Query.baseRegistry.get(baseAlias.base);
    ...
    for (let joinObject of Object.values(this.data.bases).filter(val => val.fromBaseAlias)) {
      ...
      let joinFrom = Query.baseRegistry.get(fromObject.base);
  ...
  compileWhere(context, where) {
    if (where.combine) {
      let Query = context.engine.plugin.get('Query');
