...
class Attribute extends Table {
   ...
   * The entity is in form.entity.
   */
  async formInit(formInstance, elementInstance, attribute) {
    ...
    elementInstance
      .addInstance(Static.newInstance(formInstance.context, {
        name: `${elementInstance.name}[id]`,
        data: {
          defaultValue: String(attribute.id || 0),
        },
      }))
      .addInstance(Hidden.newInstance(formInstance.context, {
        name: `${elementInstance.name}[weight]`,
        data: {
          defaultValue: String(attribute.weight || 0),
        },
      }));
  }
   ...
   * needs to be validated.  The entity is in form.entity.
   */
  async formValidate(formInstance) {}

  /**
   * This function is called when a form has passed validation and the attribute
   * needs to be added to the form.entity object.
   */
  async formSubmit(formInstance) {}
}
