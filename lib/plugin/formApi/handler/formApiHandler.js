...
class FormApiHandler extends Handler {
  async init(context) {
    ...
    form = new (FormApi.getForm(formId))(buildState, ivBase);
    await form.init(context, post);
    ...
    // Run Submit
    await form.submit(context);
    for (let element of form.getAllElementsRecursive()) {
      await element.submit ? element.submit(context) : Promise.resolve();
    }
