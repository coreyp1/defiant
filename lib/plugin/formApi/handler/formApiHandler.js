...
class FormApiHandler extends Handler {
  init(context) {
    ...
    return co(function*(self) {
      ...
      let fileUploadEntity = context.engine.pluginRegistry.get('Orm').entity.get('fileUpload');
      ...
      for (let fieldName of files) {
        for (let uuid in post[fieldName]) {
          if (!post[fieldName][uuid].processed) {
            ...
            let fileUpload = context.engine.pluginRegistry.get('Orm').entity.get('fileUpload');
