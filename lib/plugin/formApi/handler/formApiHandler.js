...
class FormApiHandler extends Handler {
  async init(context) {
    ...
    let dummyForm = new Form(context.engine).newInstance(context);
    await dummyForm.init({ivBase});
    ...
    const formInstance = form.newInstance(context);
    await formInstance.init({buildState, ivBase});
