...
class FormInstance extends CollectionInstance {
  ...
  async init(data={}) {
    ...
    this.ivBase = data.ivBase || uuid.v4();
    this.ivCounter = 0;

    // Determine the post information.
    this.context.post[this.name] = this.context.post[this.name] || {};
  ...
  setError(elementName, message) {
    ...
    this.context.engine.pluginRegistry.get('FormApi').setError(this.context, this.name, elementName, message);
  }
