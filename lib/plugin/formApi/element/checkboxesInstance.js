...
class CheckboxesInstance extends ElementInstance {
  async init(data={}) {
    ...
    for (let entry of (this.data.checkboxes || [])) {
      ...
      if (this.context.post[this.formInstance.id] && this.context.post[this.formInstance.id][this.name] && this.context.post[this.formInstance.id][this.name][entry.value]) {
