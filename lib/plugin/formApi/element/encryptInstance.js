...
class EncryptInstance extends HiddenInstance {
  async init(data={}) {
    ...
    this.encrypted = Cipher.encryptSigned(JSON.stringify(this.plainText), '', this.context.session.formApiKeyRaw, this.formInstance.getIV(this.name + this.ivCounter));
  ...
  async validate() {
    ...
    let post = (this.context.post && this.context.post[this.formInstance.name]) ? this.context.post[this.formInstance.name] : {};
