...
class Checkboxes extends Element {
  init(context) {
    ...
    for (let entry of (this.data.checkboxes || [])) {
      ...
      label.addElement(checkbox).addElement(new TagPair({
        tag: 'span',
        attributes: {
          class: 'label-text',
        },
      ...
      let checkboxContainer = new Element(checkboxName).addElement(label);
      checkboxContainer.wrap = new TagPair({
        tag: 'div',
        attributes: {
          class: ['form-checkbox', checkboxName],
        }
      });
      this.addElement(checkboxContainer);
    }
