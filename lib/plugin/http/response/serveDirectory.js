...
class ServeDirectory /*extends Response*/ {
  ...
  constructor(data) {
    ...
    this.serveStatic = promisify(serveStatic(data.target, merge({redirect: false, maxAge: '1d'}, data.fileOptions || {})));
  ...
  commit(context) {
    ...
    return co(function*(self){
      ...
      let pathSkipLength = self.path.split('/').length;
      ...
      if (!err && self.data.directoryOptions) {
        request.__defiant_discarded = self.path;
