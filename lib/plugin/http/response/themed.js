...
class Themed extends Response {
  ...
  commit() {
    return co(function*(self){
      // Set the layout.
      yield self.context.layout.init(self.context, {content: self.data.content});
      self.data.content = self.context.layout.commit();

      // Render the page and send it to the user.
      let renderable = new (self.context.theme.getRenderable('Page'))(self.data);
      yield renderable.init(self.context);

      // Include the Head CSS.
      self.data.head = self.data.head || "";
      for (let css of self.context.cssRegistry.getOrderedElements()) {
        if (css.data) {
          // Include inline CSS.
      ...
      for (let js of self.context.jsRegistry.getOrderedElements()) {
        if (js.data) {
          // Include inline JS.
      ...
      for (let js of self.context.jsFooterRegistry.getOrderedElements()) {
        if (js.data) {
          // Include inline JS.
      ...
      self.context.response.statusCode = self.statusCode;
