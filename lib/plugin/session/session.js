...
class Session extends Plugin {
  constructor(engine) {
    super(engine);
    this.sessions = {};
    engine.registry.http.incoming.set({id: 'sessionLoad', weight: -20, incoming: (...args) => this.incomingLoadSession(...args)});
    ...
    engine.registry.http.incoming.set({id: 'sessionReader', weight: -25, incoming: ({request, response}) => co(function*() {
